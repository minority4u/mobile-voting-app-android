...
namespace MSO.StimmApp.Views.Controls
{
    public class CardStackView : ContentView
    {
        ...
        public static readonly BindableProperty ItemsSourceProperty =
            BindableProperty.Create(nameof(ItemsSource), typeof(System.Collections.ICollection), typeof(CardStackView), null,
            propertyChanged: OnItemsSourcePropertyChanged);

        public ObservableCollection<AppStimmer> ItemsSource
        {
            get
            {
                return (ObservableCollection<AppStimmer>)GetValue(ItemsSourceProperty);
            }
            set
            {
        ...
        void Setup()
        {
            ...
            for (int i = 0; i < Math.Min(NumCards, ItemsSource.Count); i++)
            {
                ...
                var card = cards[i];
                card.Photo.Source = ImageSource.FromFile(ItemsSource[itemIndex].Picture);
                card.IsVisible = true;
        ...
        void ShowNextCard()
        {
            ...
            if (itemIndex < ItemsSource.Count)
            {
                ...
                // set the data
                topCard.Photo.Source = ImageSource.FromFile(ItemsSource[itemIndex].Picture);
