...
namespace MSO.StimmApp.Views.Pages
{
    ...
    public partial class AppStimmerEditorPage : ContentPage
    {
        ...
        {
            this.ImageView.Opacity = this.ImageView.Scale = 0;
            this.TopBarGradientFrame.Opacity = this.ImageView.Scale = 0;
            this.NavigationBarFrame.Opacity = this.NavigationBarFrame.Scale = 0;
            this.ImageTitleLabel.Opacity = this.ImageTitleLabel.Scale = 0;
            this.SubmitButton.Opacity = this.SubmitButton.Scale = 0;

            //var contentView = this.AttachmentsScrollView.FindByName<ContentView>("AttachmentsContentView");

            this.AppstractView.Opacity = this.AppstractView.Scale = 0;
            this.MediaHeaderView.Opacity = this.MediaHeaderView.Scale = 0;
            this.AttachmentsGrid.Opacity = this.AttachmentsGrid.Scale = 0;
            this.DescriptionHeader.Opacity = this.DescriptionHeader.Scale = 0;
            this.DescriptionView.Opacity = 0;
            this.DescriptionView.Scale = 0.8;

            await Task.Delay(Animator.DelaySpeed);

            Animator.SimpleFade(this.ImageView, Animator.FadeType.In);
            Animator.SimpleFade(this.TopBarGradientFrame, Animator.FadeType.In);
            Animator.SimpleFade(this.ImageTitleLabel, Animator.FadeType.In);
            Animator.SimpleFade(this.SubmitButton, Animator.FadeType.In);
            await Animator.SimpleFade(this.NavigationBarFrame, Animator.FadeType.In);

            Animator.SimpleFade(this.AppstractView, Animator.FadeType.In);
            Animator.SimpleFade(this.MediaHeaderView, Animator.FadeType.In);
            Animator.SimpleFade(this.AttachmentsScrollView, Animator.FadeType.In);
            Animator.SimpleFade(this.AttachmentsGrid, Animator.FadeType.In);
            Animator.SimpleFade(this.DescriptionHeader, Animator.FadeType.In);
            Animator.SimpleFade(this.DescriptionView, Animator.FadeType.In);


            var appStimmerAttachmentsAppearingAction = new Action<double>((v) =>
                {
                    AttachmentsScrollView.ItemSpacing = (-100 + v);
                    AttachmentsScrollView.Opacity = v / 100;
                });

            var animation = new Animation(appStimmerAttachmentsAppearingAction, 0, 100, Easing.Linear);
            animation.Commit(this, "ApPStimmerAttachmentsAnimation", 1, 1000, Easing.Linear);
        }
